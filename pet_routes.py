...
def get_pet_images(pet_id, pet_type, stage):
    """获取宠物指定阶段的图片，用 pet_id 确定选哪张，每只宠物固定不同"""
    if stage == 'egg':
        return []
    ...
    if not images:
        return []
    # 用 pet_id 决定选哪张，同一只宠物永远显示同一张，不同宠物不同
    idx = pet_id % len(images)
    return [os.path.join('images', 'pets', pet_type, images[idx])]
...
def child_pet():
    ...
    # 每只宠物固定一张图片（用 pet_id 决定）
    pet_images = {}
    for pet in pets:
        images = get_pet_images(pet['id'], pet['type'], pet['stage'])
        if images:
